...
const AdminClaimDetailPage: React.FC = () => {
    ...
    const handleStatusChange = async (newStatus: ClaimStatus) => {
        ...
        setIsUpdating(true);
        try {
            await mockDb.updateDoc('claims', id, { status: newStatus });
        ...
        <div className="max-w-4xl mx-auto p-4 md:p-8">
            ...
            <div className="bg-white shadow-lg rounded-lg overflow-hidden">
                <header className="bg-gray-800 text-white p-6">
                    ...
                    <p className="mt-2 text-gray-300">Client: {claim.userEmail}</p>
                </header>
                <div className="p-6 space-y-6">
                    {/* Claim details render here */}
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <h3 className="font-semibold text-gray-700">Policy ID</h3>
                            <Link to={`/admin/policy/${claim.policyId}`} className="text-primary hover:underline">{claim.policyId}</Link>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        ) : <p className="text-gray-500 text-sm">No evidence was uploaded.</p>}
                    </div>

                    <div className="bg-gray-100 p-4 rounded-lg">
                        <h3 className="font-semibold text-gray-800 mb-4">Admin Actions</h3>
                        <div className="flex flex-wrap gap-4">
                            {claim.status === ClaimStatus.SUBMITTED && <Button onClick={() => handleStatusChange(ClaimStatus.UNDER_REVIEW)} isLoading={isUpdating} variant='accent'>Start Review</Button>}
                            {claim.status === ClaimStatus.UNDER_REVIEW && <Button onClick={() => handleStatusChange(ClaimStatus.APPROVED)} isLoading={isUpdating} variant='primary'>Approve Claim</Button>}
                            {claim.status === ClaimStatus.APPROVED && <Button onClick={() => handleStatusChange(ClaimStatus.PAID)} isLoading={isUpdating} variant='primary'>Mark as Paid</Button>}
                            {(claim.status === ClaimStatus.SUBMITTED || claim.status === ClaimStatus.UNDER_REVIEW) && <Button onClick={() => handleStatusChange(ClaimStatus.REJECTED)} isLoading={isUpdating} variant='secondary'>Reject Claim</Button>}
                        </div>
                    </div>
                </div>
            </div>
